...
@section('title','Lawxblog - Postların Kateqoriyaya əlavə edilmə bölməsi')
@section('content')
                            ...
                            <div class="widget-body">
                                <form enctype="multipart/form-data" action="{{ route('admin.nc.save', $entry->id) }}" method="POST" class="form-horizontal">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-6">
                                            <select name="post_id" id="select2-demo-1" class="form-control select2-hidden-accessible" data-plugin="select2" tabindex="-1" aria-hidden="true">
                                                <option>Xəbər Seç</option>
                                            @foreach($news as $new)
                                                    <option value="{{ $new->id }}" {{ $new->id == $entry->post_id ? 'selected' : '' }}>
                                                        {{ $new->title }}
                                                    </option>
                                                @endforeach
                                        ...
                                        <div class="col-sm-6">
                                            <select name="cat_id" id="select2-demo-1" class="form-control select2-hidden-accessible" data-plugin="select2" tabindex="-1" aria-hidden="true">
                                                ...
                                                @foreach($categorys as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == $entry->cat_id ? 'selected' : '' }}>{{ $category->title }}</option>
                                                @endforeach
                                            </select>
                                        </div><!-- END column -->
                                    </div>
